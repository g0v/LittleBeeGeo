...
ad_versions = [
    ...
    "馬政府 正在騙我們的事",
    "20140429_自經5QA",
    "電信業是中樞命脈 -引用自林盈達教授_QR-02(更改版)",
    "140409圖解兩岸監督條例_彩色",
    "菜市場阿公阿罵版",
    "服貿到底關我什麼事",
    "20140507_第六彈_自由經濟是特效藥嗎",
    "20140507_第六彈_政府不說的服貿真相",
    "反服貿不等於反開放",
]

_ad_version_map = {
    "Bee140405+醫療三大問題": ["菜市場阿公阿罵版", "服貿x醫療_140405-2"],
}
...
def _parse_csv(data):
    ...
    df['google_info'] = df.apply(lambda x: _parse_google_info(dict(x), funnel_dict), axis=1)
    ...
    df = df[['csv_key', 'county', 'town', 'deliver_time', 'deliver_date', 'user_name', 'address', 'google_info', 'ad_versions', 'count']]
...
def _parse_google_info(x, funnel_dict):
    x = _unicode_dict(x)
    the_address = x.get(u'路名（區域）', '')
...
def _parse_ad_versions(x, funnel_dict):
    ...
    #cfg.logger.debug('the_versions: %s', the_versions)

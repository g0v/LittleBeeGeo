...
def _git_clone(target_host, branch='master'):    
    with cd("/srv"):
        try:
            run("git clone -b " + branch + " https://github.com/g0v/LittleBeeGeo")
        except:
...
def _set_virtualenv(target_host, virtualenv_dir='__'):
    with cd("/srv/LittleBeeGeo/LittleBeeGeo_backend"):
        run("./scripts/init_pcreate.sh " + virtualenv_dir)


def _install_packages(target_host, virtualenv_dir='__'):
    with cd("/srv/LittleBeeGeo/LittleBeeGeo_backend"), prefix("source " + virtualenv_dir + "/bin/activate"):
        run("pwd")
        run("which python")
...
def _install_production_ini(target_host, n_proc, ini_filename='staging.ini', virtualenv_dir='__'):
    ...
    with cd("/srv/LittleBeeGeo/LittleBeeGeo_backend"):
        run("cp " + ini_filename + ' /etc/LittleBeeGeo/production.ini')


def _install_log_dir(target_host):
    try:
        sudo("mkdir -p /var/log/LittleBeeGeo")
    except:
        print(red("[ERROR] unable to mkdir -p /var/log/LittleBeeGeo"))

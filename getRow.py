...
###try if c = gspread.Client(auth=('user@example.com', 'qwertypassword')) needed to be login on g-spread first or not
...
##NOTICE::
##open_by_url api is not working for the new version of google spreadsheet now, so please use opne by title api.
...
##and you can know how many column there is(col_count) or by the length of a row, and then next time you know how many rows you get (by head(cusor is moved) to get_all)

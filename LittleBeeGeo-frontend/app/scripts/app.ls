...
angular.module 'LittleBeeGeoFrontend' <[ ngRoute ngCookies ngResource ngGrid ui.map ui.event ui.bootstrap ui.select2 angularFileUpload ]>
  .config <[ $routeProvider $locationProvider ]> ++ ($routeProvider, $locationProvider, config) ->
    $routeProvider
      ...
      .when '/sheet' templateUrl: '/views/sheet.html'
    # Catch all
